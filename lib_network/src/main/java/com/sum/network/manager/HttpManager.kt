...
import com.sum.network.constant.URL_DOMAIN
import com.sum.network.constant.URL_DOMAIN_URL
import com.sum.network.error.NoNetWorkException
import com.sum.network.error.ERROR
import com.sum.network.interceptor.BaseUrlInterceptor
import com.sum.network.interceptor.CookiesInterceptor
import com.sum.network.interceptor.DataEncryptInterceptor
import com.sum.network.interceptor.HeaderInterceptor
import com.sum.network.interceptor.logInterceptor
...
object HttpManager {
    ...
    fun getBaseUrl(): String {
        return KVStorage.getString(URL_DOMAIN, URL_DOMAIN_URL, "") ?: ""
    }
    ...
    private fun initOkHttpClient(): OkHttpClient {
        ...
        build.addInterceptor(BaseUrlInterceptor())
//        build.addInterceptor(logInterceptor)
        ...
        if (SumAppHelper.isDebug()) {
            ...
            logInterceptor.level = Level.BASIC
        }
        build.addInterceptor(logInterceptor)
        //网络状态拦截
